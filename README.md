...
阶段 3 的主进程编排、连续规则采样、显式屏幕授权、窗口能力隔离、AI 二次确认、`safeStorage` 秘密存储和兼容多模态适配器已完成自动化收口。Windows sidecar、真实桌面 Electron、用户主动选源后的媒体流建立/停止、进程清理、退出后磁盘隐私扫描、临时假密钥的真实 `safeStorage` 保存/重开/清除，以及应用真实低分辨率单帧中的主窗/悬浮窗内容保护均已通过。真实使用发现的弹窗自动枚举卡顿、桌面手工巡查竞态、超高会话设置弹层和阻断式巡查反馈也已修正：屏幕列表改为用户点击后加载，会话设置正文独立滚动且操作栏常驻，中途巡查结果与置顶悬浮窗在 5 秒后由主进程自动收起并继续计时，不要求用户返回主窗确认。阶段 3 隔离验收辅助也已落地：它使用临时 `userData`、只接受固定本机假令牌与严格请求形状的 `127.0.0.1` mock AI、按实际巡查次数推进的固定节点、低敏证据摘要和退出后精确清理；会话中可在用户再次明确选屏后重新授权，验收模式硬性禁止发送窗口标题。正式模式仍使用随机巡查，也不会自动选择屏幕。当前仍需完成一次真实 25 分钟人工场，以及全屏、系统停止共享、截图窗销毁、休眠恢复、125%/150% DPI 和多显示器矩阵；当前机器可做全屏与休眠，修改 DPI 需要用户同意，多显示器需要真实第二块屏幕。在这些门禁全部通过前，不宣称阶段 3 完成，也不进入阶段 4 的第三方角色素材生产；应用安装包与签名继续属于后续发布阶段。
...
`diagnose:stage3:startup` 使用随机隔离 `userData` 连续冷启动（默认 3 次，可传 `--runs=N`），只确认能到达会话设置界面，不请求屏幕权限、不读取正式数据库。

`rehearse:stage3:b1` 是 B1 短预演（约 4 分钟，需用户参与真实前台切换）：只验证 2:00 允许规则节点与 3:00 连续禁止规则节点，任一节点不符立即结束并输出低敏诊断；可用 `--allow=` / `--block=` 指定允许/禁止进程（默认 notepad / mspaint）。该预演通过也不算 G3-B。

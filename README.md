...
阶段 3 的主进程编排、连续规则采样、显式屏幕授权、窗口能力隔离、AI 二次确认、`safeStorage` 秘密存储和兼容多模态适配器已完成自动化收口。Windows sidecar 已通过可重复自包含构建、生产路径解析与显式真实探针测试，真实桌面权限下的 Electron 短时启动、退出和进程清理也已通过。当前仍需完成用户主动选源后的真实捕获、内容保护、多屏/DPI、捕获后的磁盘隐私复扫和 25 分钟人工验收。在这些门禁全部通过前，不宣称阶段 3 完成，也不进入阶段 4 的第三方角色素材生产；应用安装包与签名继续属于后续发布阶段。

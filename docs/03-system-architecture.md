...
默认 420×236、置顶，仅在巡查和结果反应时显示。窗口启用内容保护，使其不进入 Electron/Windows 的正常屏幕捕获结果；全程鼠标穿透，不夺取学习软件焦点。每次预览由主进程设置 5 秒自动隐藏上限，新结果会重置该上限，显式隐藏、阶段切换和退出会取消计时器，避免悬浮窗持续遮挡或要求用户返回主窗确认。

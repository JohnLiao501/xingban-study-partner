...
候选引用一个视频、一个或多个台词，并可设置 `weight` 与 `minimumTrust`。正式伙伴运行时须按权重选择满足信赖阈值且未在冷却期的候选。N1 开发预览只确定性选择当前最高已解锁门槛并在未解锁时降级到零信赖候选；权重与冷却轮换仍属于阶段 3 关闭后的 P1-001 正式验收范围。
...
## 10. N2 安装边界（2026-09-05）

- 桌面“导入伙伴包”可选择 ZIP 或文件夹。ZIP 根层必须直接包含 `manifest.json` 与 `assets/`，不自动剥除外层目录。仅接受普通 JSON 清单与 `.webp`、`.mp4`、`.ogg` 媒体；目录导入执行相同白名单，根层额外文件也会拒绝。
- ZIP 最大 512 MiB，展开总量最大 1 GiB，单媒体最大 256 MiB，清单最大 1 MiB，条目（含目录）最多 2048 个。相对路径最多 200 字符、16 层；拒绝绝对路径、盘符、反斜杠、空段、点段、尾点、Windows 设备名、ADS、大小写路径别名、符号链接、设备与 reparse 条目。加密 ZIP 不支持。
- 使用逐条解压、实际字节计数与 CRC32，再执行 Schema、声明文件集合、SHA-256 和媒体文件头校验。当前没有解码器级编码/帧率/时长探测，不能把文件头通过描述成完整媒体 QA。
- 在安装根下的随机 `.install-*` 目录准备和校验，用同卷 `rename` 发布到 `<partnerId>/<packVersion>`；同版本拒绝覆盖，新版本保留旧版本。SQLite 登记失败撤回本次新目录，进度表不改变。正常取消、失败、120 秒超时与应用正常退出会清理本次暂存；导入期间禁止开始会话、切换伙伴或重复导入。
- 文件系统与 SQLite 不是跨介质事务。断电/强制杀进程可能留下未登记版本或 `.install-*`；这些目录不会自动激活，重试同版本会保护性报冲突，需要核对后恢复，不能自动删除既有版本。正常失败回滚与这种崩溃恢复边界应分别记录。
- 新增运行时 `yauzl 3.4.0`（MIT，npm 解包 109,901 字节）及其 `pend`（MIT，本地 6,015 字节）；`@types/yauzl` 仅用于开发。选择原因是现有 Electron 内部解压器不支持条目限额且允许符号链接；新依赖只读 ZIP，写盘、限额、路径与校验仍由本应用控制，没有新增原生编译工具链。参考：[yauzl 官方文档](https://github.com/thejoshwolfe/yauzl)。

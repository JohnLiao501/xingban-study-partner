...
| 阶段 3 | Windows 前台探针、自动规则判断、真实单帧捕获、兼容 AI | 进行中；真实捕获与隐私门禁已通过，剩余环境矩阵和 25 分钟人工场 |
...
- `pnpm test` 通过：20 个测试文件，165 项通过、1 项默认跳过（真实 sidecar 专项）。
...
3. 当前机器只提供单显示器与 100% 系统 DPI；多显示器、125%/150% DPI、全屏应用、系统停止共享、捕获窗销毁和休眠恢复尚未完成人工验收。
4. 真实单帧取帧、媒体流停止并退出后的应用数据、近三小时临时目录与 SQLite 零截图/base64 扫描已通过；25 分钟人工模拟学习仍未完成。

当前只支持目录导入，ZIP 安装继续留在阶段 3 之后；通用多伙伴原型也不等于私有伙伴包已验收。工作区有 Git 元数据，当前主线分支为 `master`；每次接手仍必须实时检查分支、工作树与远端状态，保留用户或其他 Agent 的现场修改，不得把本文中的历史描述当成提交、推送或外部交付证据。
...
- [ ] S3-016 完成内容保护、多屏/DPI、停止共享和真实 Electron 验收（真实选源、单帧内容保护、媒体流停止与清理已通过；多屏、125%/150% DPI、全屏和休眠恢复仍待验证）。
...
2026-08-30 已重新执行当前门禁：20 个测试文件中 165 项通过、1 项默认跳过；真实 sidecar 专项 5 项通过；类型检查、生产构建、浏览器桌面/窄屏 QA、真实桌面 Electron、用户主动选源后的媒体流建立/停止、退出清理与零截图残留扫描均通过。判定政策 v1 的默认私人通讯提醒、严格模式两帧确认和语义矛盾降级已有自动化证据；临时假密钥的真实 `safeStorage` 保存、数据库重开解密、清除与再次重开确认也已通过。真实 768×432 内容保护帧同时证明原生对照窗可见、主窗与悬浮窗的受保护标记不可见，结果为 `passed`；取帧后的应用数据、SQLite 与相关临时目录复扫无截图/base64 残留。阶段 3 仍不能标记完成，因为当前硬件未覆盖的多屏、125%/150% DPI、全屏与休眠恢复，以及 25 分钟人工模拟尚未取得完整证据。

重新验收至少包括：默认不共享、本地规则连续 20 秒、AI 15 秒二次确认、截图窗口 sender 隔离、HTTPS/本机 URL 限制、API 失败不处罚、流/sidecar/进程清理、真实内容保护截图与磁盘零残留。

下一批不直接开始角色生产，按 `docs/11-stage-3-completion-plan.md` 执行：

1. 先实现隔离用户数据、脚本化本机 mock AI、确定性巡查节点和低敏验收摘要，避免污染真实历史或依赖真实密钥。
2. 自动预检通过后，只让用户参与一次真实 25 分钟场；同一场覆盖停止共享和一次重新授权，不再单独反复索要授权。
3. 在当前机器可执行全屏和休眠恢复；125%/150% DPI 需要用户同意临时修改系统缩放；多显示器仅在真实第二显示器可用时验收，不用截图拼接或虚拟假数据冒充。
4. 所有未完成单元取得证据后，复跑 `typecheck`、全部测试、生产构建、真实探针、进程清理和隐私扫描，再勾选 S3-016～S3-018。

## 8. 当前接手边界

- 可保留：阶段 0～2、伙伴包通用协议、数据库 v1、Windows sidecar 与阶段 3 各接口的现有实现基础。
- 已完成自动化修正：显式授权、连续采样、AI 二次确认、IPC/网络安全和真实桌面基础启动。
- 下一批先完成验收辅助与隔离环境，再运行一次 25 分钟人工场；随后按环境条件补齐多屏、125%/150% DPI、全屏与休眠恢复。应用单帧内容保护和取帧后隐私复扫已经通过，不要重复要求用户单独授权。

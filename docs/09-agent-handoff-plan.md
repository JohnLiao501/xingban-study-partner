...
B1 已通过。用户于 2026-09-03 决定把 B2～B5 延期到通用应用代码完成后的最终关闭阶段；下一步不再进入 B2，改走下述 N1～N4 非 B 开发路线。
...
- N1：用两份原创中性伙伴夹具收口 P1-001 的通用列表、选择、场景、动作降级和关系进度隔离。
- N2：实现 P1-002 的 ZIP 安装安全闭环，包括路径穿越、符号链接、资源上限、Schema、原子安装和失败回滚。
...
下一批不直接开始角色生产，也不执行 B 组验证，按 `docs/11-stage-3-completion-plan.md` 的 N1～N4 推进。通用应用代码完成后，再按 B2～B5 回补真实证据；未全部通过时不得勾选 S3-016～S3-018 或开始 P1-004。

## 8. 当前接手边界

- 可保留：阶段 0～2、伙伴包通用协议、数据库 v1、Windows sidecar 与阶段 3 各接口的现有实现基础。
- 已完成自动化修正：显式授权、连续采样、AI 二次确认、IPC/网络安全和真实桌面基础启动。
- G3-A 验收辅助与隔离环境已经完成自动门禁；B2～B5 按用户决定冻结到通用应用代码完成后的最终关闭阶段。下一批执行 N1 多伙伴公共底座，不运行 25 分钟人工场或环境矩阵。应用单帧内容保护和取帧后隐私复扫已经通过，不要重复要求用户单独授权；真实 runner 中断清理仍在最终 G3-G 复验。

...
- `pnpm test` 通过：18 个测试文件，150 项通过、1 项默认跳过（真实 sidecar 专项）。
...
1. 主窗口与悬浮窗已经启用内容保护，系统截图工具无法捕获应用窗口；但尚未通过应用生成的真实低分辨率单帧确认两者不会进入巡查内容。
2. 仍需在真实捕获验收中复测 `safeStorage` 保存/清除与完整退出链路。
3. 多显示器、125%/150% DPI、全屏应用、系统停止共享、捕获窗销毁和休眠恢复尚未完成人工验收。
4. 真实媒体流停止并退出后的应用数据、近两小时临时目录与 SQLite 零截图/base64 扫描已通过；生成真实巡查单帧后的复扫与 25 分钟人工模拟学习仍未完成。
...
- [ ] S3-016 完成内容保护、多屏/DPI、停止共享和真实 Electron 验收（真实选源、媒体流停止与清理已通过；单帧内容保护和多屏/DPI 仍待验证）。
...
2026-08-30 已重新执行当前门禁：18 个测试文件中 150 项通过、1 项默认跳过；真实 sidecar 专项 5 项通过；类型检查、生产构建、浏览器桌面/窄屏 QA、真实桌面 Electron、用户主动选源后的媒体流建立/停止、退出清理与零截图残留扫描均通过。仍不能把阶段 3 标记完成，因为应用真实单帧中的内容保护、多屏/DPI、`safeStorage` 保存/清除与 25 分钟人工模拟尚未取得证据。
...
- 下一批继续真实验收：应用单帧内容保护、多屏/DPI、`safeStorage` 保存/清除、单帧后隐私复扫和 25 分钟人工场。

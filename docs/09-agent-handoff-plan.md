...
- `pnpm test` 通过：29 个测试文件，198 项通过、1 项默认跳过（真实 sidecar 专项）。
...
下一批不直接开始角色生产，按 `docs/11-stage-3-completion-plan.md` 的 B0～B5 执行：

1. B0 先解决或稳定复现会话前 GPU/renderer `launch-failed`；连续 3 次隔离冷启动到设置界面后才进入真实节点，不以默认关闭硬件加速掩盖根因。
2. B1 用约 3 分钟有效计时预演 2:00 允许规则与 3:00 连续禁止规则；真实前台切换由用户完成，任一节点不符立即停止，不继续浪费完整 25 分钟。
3. B1 通过后再执行一次完整 B2/G3-B；同一场覆盖停止共享、一次重新授权与暂停恢复，不重复索要额外授权。
4. B3 分开执行全屏、系统停止共享、截图窗销毁与休眠恢复；B4 的 125%/150% DPI 需用户同意，多显示器只接受真实第二显示器证据。
5. B5 复跑 `typecheck`、全部测试、生产构建、真实探针、进程清理和隐私扫描，核对文档与实现后再勾选 S3-016～S3-018。

## 8. 当前接手边界

- 可保留：阶段 0～2、伙伴包通用协议、数据库 v1、Windows sidecar 与阶段 3 各接口的现有实现基础。
- 已完成自动化修正：显式授权、连续采样、AI 二次确认、IPC/网络安全和真实桌面基础启动。
- G3-A 验收辅助与隔离环境已经完成自动门禁；下一批运行一次 25 分钟人工场，随后按环境条件补齐全屏、系统停止共享、截图窗销毁、休眠、DPI 与多屏。应用单帧内容保护和取帧后隐私复扫已经通过，不要重复要求用户单独授权；真实 runner 中断清理仍在 G3-G 复验。
